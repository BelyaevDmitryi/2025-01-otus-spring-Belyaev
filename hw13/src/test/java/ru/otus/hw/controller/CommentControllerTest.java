...
class CommentControllerTest {
    ...
    @DisplayName("отдать страницу с комментариями для книги с моделью")
    @Test
    void shouldRenderCommentForBookPageWithModel() throws Exception {
        ...
        mvc.perform(get("/comment/book/1")
                        .with(user("lib")
                                .authorities(new SimpleGrantedAuthority("ROLE_COMMENT_EDITOR")))
                )
    ...
    @DisplayName("отдать страницу для редактирования комментария с моделью")
    @Test
    void shouldRenderEditCommentPageWithModel() throws Exception {
        List<CommentDto> commentDtoList = createCommentDtoList();
    ...
    @DisplayName("обновить комментарий и перенаправить на страницу с комментариями для книги")
    @Test
    void shouldSaveEditCommentAndRedirectCommentForBookPage() throws Exception {
        CommentDto commentDto = new CommentDto();
        commentDto.setId(1);
        commentDto.setText("updated_comment_text");
        ...
        mvc.perform(post("/comment")
                        ...
                        .param("id", "1")
                        .param("bookId", "1")
                        .param("text", "updated_comment_text"))
                .andExpect(view().name("redirect:/comment/book/1"));

        verify(commentService, times(1)).update(commentDto);
    }

    @DisplayName("удалить комментарий и перенаправить на страницу с комментариями для книги")
    @Test
    void shouldDeleteCommentAndRedirectCommentForBookPage() throws Exception {
        List<CommentDto> commentDtoList = createCommentDtoList();
        CommentDto commentDto = commentDtoList.get(0);
        ...
        mvc.perform(post("/comment/1/delete")
                        .with(user("user")
                                .authorities(new SimpleGrantedAuthority("ROLE_BOOK_EDITOR")))
                        .with(csrf())
                )
                .andExpect(view().name("redirect:/comment/book/1"));
    ...
    @DisplayName("отдать страницу для создания комментария с моделью")
    @Test
    void shouldRenderNewCommentPageWithModel() throws Exception {
    ...
    @DisplayName("сохранить комментарий и перенаправить на страницу с комментариями для книги")
    @Test
    void shouldSaveNewCommentAndRedirectCommentForBookPage() throws Exception {
        CommentDto commentDto = new CommentDto();
        commentDto.setId(1);
    ...
    @DisplayName("redirect на страницу редактирования из-за пустого комментария")
    @Test
    void shouldNotSaveBookWithEmptyTitleAndRedirectEditPage() throws Exception {
        ...
        mvc.perform(post("/comment")
                        .with(user("user").authorities(new SimpleGrantedAuthority("ROLE_BOOK_EDITOR")))
                        .with(csrf())
                )
                .andExpect(model().attributeHasFieldErrorCode("comment", "text", "NotBlank"))
                .andExpect(view().name("comment_edit"));
