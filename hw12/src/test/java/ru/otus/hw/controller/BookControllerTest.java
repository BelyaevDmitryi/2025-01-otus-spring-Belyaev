...
class BookControllerTest {
    ...
    void shouldRenderMainPageWithModel() throws Exception {
        ...
        mvc.perform(get("/").with(user("user").authorities(new SimpleGrantedAuthority("BOOK_EDITOR"))))
...
//                .andExpect(status().isOk());
    ...
    void shouldDeleteBookAndRedirectMainPage() throws Exception {
        mvc.perform(post("/book/1/delete")
                        .with(user("lib").authorities(new SimpleGrantedAuthority("COMMENT_EDITOR")))
                        .with(csrf())
                )
    ...
    void shouldRenderNewBookPageWithModel() throws Exception {
        ...
        mvc.perform(get("/book").with(user("user").authorities(new SimpleGrantedAuthority("BOOK_EDITOR"))))
                .andExpect(view().name("book_edit"))
    ...
    void shouldRenderErrorPage() throws Exception {
        ...
        mvc.perform(get("/book/1").with(user("user").authorities(new SimpleGrantedAuthority("BOOK_EDITOR"))))
